 ...
 *
 *  Modified for listcerts.efi build environment  
 *
 */
...
int main(int argc, char **argv)
{
	...
	if (argc != 4) {
		fprintf(stderr, "Usage: %s <grammar-file> <c-file> <hdr-file>\n",
			argv[0]);
		exit(2);
	}
...
static void render(FILE *out, FILE *hdr)
{
	...
	fprintf(hdr, "/*\n");
	fprintf(hdr, " * Automatically generated by ASN1 compiler.  Do not manually edit!\n");
	fprintf(hdr, " *\n");
	fprintf(hdr, " * ASN.1 parser for %s\n", grammar_name);
	fprintf(hdr, " */\n\n");
	fprintf(hdr, "#include \"asn1_ber_bytecode.h\"\n");
	fprintf(hdr, "\n");
	...
	fprintf(out, "/*\n");
	fprintf(out, " * Automatically generated by ASN1 compiler.  Do not manually edit!\n");
	fprintf(out, " *\n");
	...
	for (action = action_list; action; action = action->next) {
		action->index = index++;
		fprintf(hdr,
			"extern int %s(void *, long, unsigned char,"
			" const void *, long);\n",
			action->name);
	}
	fprintf(hdr, "\n");
